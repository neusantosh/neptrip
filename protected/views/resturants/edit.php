							...
							<div id="tab1" class="tab-pane fade in active">
					          	...
					          	<div class="row form-group">
						            ...
						            <div class="col-md-6">
										<?php
      									...
      									?>
										 <p class="help-block">Upload Main Resturant Image.</p>
									</div>
								</div>
								...
								<div class="row form-group">
						            ...
						            <div class="col-md-6">
						            	<div class="resturant-logo">
						            		<?php
											if($record['image']!='' && file_exists('images/resturants/'.$record['image'])){
												$imageurl = Yii::app()->baseUrl.'/images/resturants/'.$record['image'];
										?>
											<img src="<?php echo $imageurl;?>" width="150px">
											<br/>
											<a href="javascript:void(0);" class="btn btn-danger removeresturantlogo" data-id="<?php echo $record['id'];?>" data-url="<?php echo Yii::app()->createUrl('resturants/deletelogo');?>" data-imagename="<?php echo $record['image'];?>">
												Remove
											</a>
										<?php		
											} 
								...
								<div class="row form-group">
						            ...
						            <div class="col-md-6">
										...
										?>
										 <p class="help-block">Upload Images of your Resturant.</p>
									</div>
								</div>
								...
								<div class="row form-group">
						            ...
						            <div class="col-md-6">
						            	...
						            	<?php 
						            		if(!empty($gallerimages) && is_array($gallerimages)){ 
						            			foreach ($gallerimages as $key => $gallerimage) {
						            				if($gallerimage['image']!='' && file_exists('images/resturants/gallery/'.$gallerimage['image'])){
						            					$imageurl = Yii::app()->baseUrl.'/images/resturants/gallery/'.$gallerimage['image'];
						            	?>
						            	<div class="resturant-imagegallery">
						            		<img src="<?php echo $imageurl;?>" width="150px">
						            		<br/>

						            		<a href="javascript:void(0);" class="btn btn-danger removeresturantimage" data-id="<?php echo $gallerimage['id'];?>" data-url="<?php echo Yii::app()->createUrl('resturants/deletegalleryimage');?>" data-imagename="<?php echo $gallerimage['image'];?>">
												Remove
											</a>
							  ...
							  <div class="row form-group">
					            <?php echo $form->labelEx($model,'resturant_type', array('class'=>'col-md-2 control-label text-left')); ?>
					            <div class="col-md-4">
					              <?php 
					              	echo $form->dropDownList($model,'resturant_type', array(
					              			''				=> '--select--',
					              			'Classic'		=> 'Classic',
					              			'Modern'		=> 'Modern',
					              			'Bar'			=> 'Bar',
					              			...
					              			'options'	=> array($record['resturant_type']=>array('selected'=>true)),
					              			'class'		=> 'form-control'
					              			));
					              	echo $form->error($model,'resturant_type');
					              ?>
					            </div>
					          </div>
			 ...
			 <div class="col-md-12">
			 	...
			 	<h3>Dining Style</h3>
					<div class="row">
						<div class="col-md-4">
							<div class="form-group">
								<?php 
								$model3->resturant_diningstyle = $resturantdiningstyle;
              						echo $form->checkBoxList($model3,'resturant_diningstyle',CHtml::listData(Diningstyles::model()->findAll(),'id','diningstyle')); 
              						echo $form->error($model3,'resturant_diningstyle');
            					?>
							</div>
						</div>
					</div>
					...
					<div class="row">
						...
						<div class="form-group">
								<?php 
								$model2->facility = $resturantfacility;
              						echo $form->checkBoxList($model2,'facility',CHtml::listData(Resturantfacilities::model()->findAll(),'id','facility')); 
					...
					<div class="row form-group">
			            <?php echo $form->labelEx($model,'opening_hour', array('class'=>'col-md-2 control-label text-left')); ?>
			            <div class="col-md-4">
				            	<div class="form-group">
					                <?php 
						              	$this->widget("ext.maskedInput.MaskedInput", array(
							                "model" 		=> $model,
							                "name" 			=> "Resturants[opening_hour]",
							                "clientOptions" => array("alias" =>  "hh:mm"),
							                'value'			=> $record['opening_hour']
            							));
						             	 echo $form->error($model,'opening_hour');
						            ?>
				            	</div>
			       		 </div>
  					</div>
  					...
  					<div class="row form-group">
			            <?php echo $form->labelEx($model,'closing_hour', array('class'=>'col-md-2 control-label text-left')); ?>
			            <div class="col-md-4">
			            	<div class="form-group">
				                <?php 
					              	 $this->widget("ext.maskedInput.MaskedInput", array(
							                "model" 			=> $model,
							                "name" 				=> "Resturants[closing_hour]",
							                "clientOptions" 	=> array("alias" =>  "hh:mm"),
							                'value'				=> $record['closing_hour']
            							));
					             	 echo $form->error($model,'closing_hour');
				            	?>
				            </div>
			            </div>
  					</div>
	...
	<script>
      ...
      function initMap() {
        ...
        var input = /** @type {!HTMLInputElement} */(
            document.getElementById('Resturants_addressonmap'));
        ...
        autocomplete.addListener('place_changed', function() {
          ...
          if (!place.geometry) {
           ...
           document.getElementById('Resturants_latitude').value = place.geometry.location.lat();
           document.getElementById('Resturants_longitude').value = place.geometry.location.lng();
