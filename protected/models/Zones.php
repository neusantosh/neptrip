 ...
 *
 * The followings are the available columns in table 'tbl_zones':
 * @property integer $id
...
class Zones extends CActiveRecord
{
	...
	public static function getZoneName($zone_id){
		$criteria 				= new CDbCriteria();
		$criteria->select 		= 'zone';
		$criteria->condition 	= "id=".$zone_id;
		$res 					= Zones::model()->findAll($criteria);
		if($res)
			return $res[0]['zone'];
		else
			return 'N/A';
